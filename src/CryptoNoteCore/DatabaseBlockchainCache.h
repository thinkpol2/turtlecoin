...
public:
  ...
  ExtractOutputKeysResult extractKeyOutputKeys(uint64_t amount, uint64_t blockIndex,
  ...
  virtual std::vector<uint32_t> getRandomOutsByAmount(uint64_t amount, size_t count,
                                                      uint64_t blockIndex) const override;
  virtual ExtractOutputKeysResult
  extractKeyOutputs(uint64_t amount, uint64_t blockIndex, Common::ArrayView<uint32_t> globalIndexes,
...
private:
  ...
  mutable boost::optional<uint64_t> topBlockIndex;
  ...
  ExtendedPushedBlockInfo getExtendedPushedBlockInfo(uint64_t blockIndex) const;

  void deleteClosestTimestampBlockIndex(BlockchainWriteBatch& writeBatch, uint64_t splitBlockIndex);
  CachedBlockInfo getCachedBlockInfo(uint64_t index) const;
  ...
  void requestDeleteSpentOutputs(BlockchainWriteBatch& writeBatch, uint64_t splitBlockIndex, const TransactionValidatorState& spentOutputs);
  std::vector<Crypto::Hash> requestTransactionHashesFromBlockIndex(uint64_t splitBlockIndex);
...
uint8_t getBlockMajorVersionForHeight(uint64_t height) const;
  ...
  std::vector<CachedBlockInfo> getLastCachedUnits(uint64_t blockIndex, size_t count, UseGenesis useGenesis) const;
  std::vector<CachedBlockInfo> getLastDbUnits(uint64_t blockIndex, size_t count, UseGenesis useGenesis) const;
};
}
