...
public:
  ...
  BlockchainWriteBatch& insertSpentKeyImages(uint64_t blockIndex, const std::unordered_set<Crypto::KeyImage>& spentKeyImages);
  ...
  BlockchainWriteBatch& insertCachedBlock(const CachedBlockInfo& block, uint64_t blockIndex, const std::vector<Crypto::Hash>& blockTxs);
  ...
  BlockchainWriteBatch& insertRawBlock(uint64_t blockIndex, const RawBlock& block);
  BlockchainWriteBatch& insertClosestTimestampBlockIndex(uint64_t timestamp, uint64_t blockIndex);
  ...
  BlockchainWriteBatch& removeSpentKeyImages(uint64_t blockIndex, const std::vector<Crypto::KeyImage>& spentKeyImages);
  ...
  BlockchainWriteBatch& removeCachedBlock(const Crypto::Hash& blockHash, uint64_t blockIndex);
  ...
  BlockchainWriteBatch& removeRawBlock(uint64_t blockIndex);
