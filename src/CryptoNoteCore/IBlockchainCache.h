...
const uint64_t INVALID_BLOCK_INDEX = std::numeric_limits<uint64_t>::max();
...
public:
  ...
  virtual BinaryArray getRawTransaction(uint64_t blockIndex, uint32_t transactionIndex) const = 0;
  virtual std::unique_ptr<IBlockchainCache> split(uint64_t splitBlockIndex) = 0;
  virtual void pushBlock(
      ...
      uint64_t blockDifficulty,
      RawBlock&& rawBlock) = 0;
  virtual PushedBlockInfo getPushedBlockInfo(uint64_t index) const = 0;
  virtual bool checkIfSpent(const Crypto::KeyImage& keyImage, uint64_t blockIndex) const = 0;
  ...
  virtual bool isTransactionSpendTimeUnlocked(uint64_t unlockTime, uint64_t blockIndex) const = 0;
  ...
  virtual ExtractOutputKeysResult extractKeyOutputKeys(uint64_t amount, uint64_t blockIndex, Common::ArrayView<uint32_t> globalIndexes, std::vector<Crypto::PublicKey>& publicKeys) const = 0;
  ...
  virtual uint64_t getBlockIndex(const Crypto::Hash& blockHash) const = 0;
  ...
  virtual std::vector<uint64_t> getLastTimestamps(size_t count, uint64_t blockIndex, UseGenesis) const = 0;

  virtual std::vector<uint64_t> getLastBlocksSizes(size_t count) const = 0;
  virtual std::vector<uint64_t> getLastBlocksSizes(size_t count, uint64_t blockIndex, UseGenesis) const = 0;

  virtual std::vector<uint64_t> getLastCumulativeDifficulties(size_t count, uint64_t blockIndex, UseGenesis) const = 0;
  ...
  virtual uint64_t getDifficultyForNextBlock(uint64_t blockIndex) const = 0;

  virtual uint64_t getCurrentCumulativeDifficulty() const = 0;
  virtual uint64_t getCurrentCumulativeDifficulty(uint64_t blockIndex) const = 0;

  virtual uint64_t getAlreadyGeneratedCoins() const = 0;
  virtual uint64_t getAlreadyGeneratedCoins(uint64_t blockIndex) const = 0;

  virtual uint64_t getAlreadyGeneratedTransactions(uint64_t blockIndex) const = 0;

  virtual Crypto::Hash getBlockHash(uint64_t blockIndex) const = 0;
  virtual std::vector<Crypto::Hash> getBlockHashes(uint64_t startIndex, size_t maxCount) const = 0;

  virtual IBlockchainCache* getParent() const = 0;
  virtual void setParent(IBlockchainCache* parent) = 0;
  virtual uint64_t getStartBlockIndex() const = 0;

  virtual size_t getKeyOutputsCountForAmount(uint64_t amount, uint64_t blockIndex) const = 0;

  virtual std::tuple<bool, uint64_t> getBlockHeightForTimestamp(uint64_t timestamp) const = 0;

  virtual uint64_t getTimestampLowerBoundBlockIndex(uint64_t timestamp) const = 0;
  ...
  virtual uint64_t getBlockIndexContainingTx(const Crypto::Hash& transactionHash) const = 0;
  ...
  virtual std::vector<uint64_t> getLastUnits(size_t count, uint64_t blockIndex, UseGenesis use,
                                             std::function<uint64_t(const CachedBlockInfo&)> pred) const = 0;
  virtual std::vector<Crypto::Hash> getTransactionHashes() const = 0;
  virtual std::vector<uint32_t> getRandomOutsByAmount(uint64_t amount, size_t count, uint64_t blockIndex) const = 0;
